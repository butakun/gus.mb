...
def Main():
	...
	curvesR1 = ReadAirfoil(open("rotor1.dat"))
	curvesR1[0][:, 0] += Cax_Nozzle + Gap
	curvesR1[1][:, 0] += Cax_Nozzle + Gap
	curvesR13D = Make3DBlade(curvesR1, [0.074, 0.0765])
	rotor1 = {"Name":"Rotor1", "Suction":curvesR13D[0], "Pressure":curvesR13D[1], "Periodicity":88}

	channelCurves = ReadAirfoil(open("channel.dat"))
	channel = {"Hub":channelCurves[0], "Shroud":channelCurves[1]}

	buf = GeomTurbo(channel, [nozzle, rotor1])
	print buf
