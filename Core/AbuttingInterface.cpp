// $Id: AbuttingInterface.cpp 306 2013-10-02 07:03:25Z kato $
...
AbuttingInterface*
AbuttingInterface::New(const BlockPatches& blockPatches, const BlockPatches& donorBlockPatches)
{
    AbuttingInterface* interface = new AbuttingInterface(blockPatches, donorBlockPatches);
...
AbuttingInterface::BlockPatches
...
void
AbuttingInterface::InitializeMapping()
{
...
void
AbuttingInterface::MapMesh(const IterationContext& iteration)
{
    std::ostream& LOG = Communicator::GetInstance()->Console();

    LOG << "AbuttingInterface::MapMesh" << std::endl;
        ...
        i != mBlockPatches.end(); ++i)
    {
        ...
        if (!block->IsStationary())
        {
            ...
            LOG << "AbuttingInterface::MapMesh: I am a rotating domain, currently at angle0 = " << angle0 * 180.0 / M_PI << " deg., patch = " << bp << std::endl;
        }
        ...
        for ( ; ; )
        {
                ...
                j != mDonorBlockPatches.end(); ++j)
            {
                ...
                if (!donorBlock->IsStationary())
                {
                    ...
                    LOG << "AbuttingInterface::MapMesh: donor is a rotating domain, currently at angle0d = " << angle0d * 180.0 / M_PI << " deg., patch = " << bpd << std::endl;
                }
            ...
            if (countBlocksDone == mDonorBlockPatches.size())
            {
                if (!allMapped)
                {
                    LOG << "AbuttingInterface: *** some interface patches are not completely matched. ***" << std::endl;
                }
                break;
            }
        }
...
void
AbuttingInterface::MapData(const Model& model, InterfaceDataAdaptorBase* adaptor) const
{
    for (BlockPatches::const_iterator i = mBlockPatches.begin();
        i != mBlockPatches.end(); ++i)
    {
...
const PlanarMapping&
AbuttingInterface::GetMapper(int blockPatchUniqueID) const
{
...
void
AbuttingInterface::DumpMappingResult(std::ostream& o) const
{
